...
require_once(ROOT_PATH . "/nusoap/nusoap.php");   //代码增加  By  www.68ecshop.com
...
if ($_REQUEST['act'] == 'list')
{
    ...
    $json   = new JSON;
    $filter = (object)$json->decode($_GET['JSON']);
    $goods    = get_goods_list($filter);
    $result = get_filter($param_str);
    $filter = $data['filter'];
    $smarty->assign('goods',$goods);
    $smarty->assign('filter',$result);
    //全部商品分类
...
/*上传多个商品图片*/
elseif($_REQUEST['act'] == 'uploads'){
    $original_img   = $image->upload_image($_FILES['goods_img']); // 原始图片
    $array[] = array('path'=>$original_img,'name'=>substr($original_img,strripos($original_img,'/')+1));
...
(
    ...
    [img] => Array
        (
            [0] => images/201806/1529365903438762058.jpg
            [1] => images/201806/1529365903480082516.jpg
            [2] => images/201806/1529365903589523197.jpg
        )
    ...
    $suppliers_id = $_SESSION['supplier_id'];
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, ghost_count, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral,exclusive, supplier_id, cost_price)" .
...
//				$zhyh='';
//			}
            $catstr =   $catstr . $pstr . $zhyh. $sa['name'];
