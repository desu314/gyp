...
//	               include_once(ROOT_PATH.'send.php');
            ...
            elseif($pay_log['order_type'] == PAY_RANK)
            {
                ...
                if(empty($res_id) && !empty($user_rank['user_id']))
                {
                    ...
                    $share_user_id = $GLOBALS['db']->getOne("select share_user_id from ".$GLOBALS['ecs']->table('users')." where user_id = ".$user_rank['user_id']);
                    $count = $GLOBALS['db']->getOne("select count(user_id) from ".$GLOBALS['ecs']->table('rank_account')." where user_id = ".$user_rank['user_id']." and is_paid = 1");
                    if($share_user_id != '' && $count == 1){
                        $money = ($user_rank['balance']+$user_rank['amount'])*APPLY_EXTRACT;//计算需要返给推荐人的金额
